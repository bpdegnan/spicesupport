#!/usr/bin/env zsh
# Run pfettrans and save output with hostname suffix
...
run_simulation() {
    echo_status "Running pfettrans on $HOSTNAME..."
    check_ngspice || return 1
    check_spice_lib || return 1
    
    # Extract metadata
    NGSPICE_VERSION=$("$NGSPICE_BIN" --version 2>&1 | head -1)
    GMIN=$(grep -i '\.option.*gmin' pfettrans.cir | sed -E 's/.*gmin[[:space:]]*=[[:space:]]*([^ ]+).*/\1/i' || echo "default")
    TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
    
    # Output filename includes gmin
    OUTFILE="pfettrans.gmin${GMIN}.${HOSTNAME}.csv"
    
    # Run ngspice
    "$NGSPICE_BIN" -b pfettrans.cir > pfettrans.out 2>&1
    
    if grep -q "Done" pfettrans.out && [[ -f "pfettrans.csv" ]]; then
        # Create output with metadata header
        {
            echo "# hostname: $HOSTNAME"
            echo "# ngspice: $NGSPICE_VERSION"
            echo "# gmin: $GMIN"
            echo "# source: pfettrans.cir"
            ...
            sed -E 's/^[[:space:]]+//; s/[[:space:]]+$//; s/[[:space:]]+/,/g' pfettrans.csv
        } > "$OUTFILE"
        rm pfettrans.csv
        ...
        echo_error "Simulation failed - check pfettrans.out"
        cat pfettrans.out
        return 1
    fi
}
...
plot_comparison() {
    ...
    CSV_FILES=(pfettrans.*.csv(N))
    
    if [[ ${#CSV_FILES[@]} -eq 0 ]]; then
        echo_error "No CSV files found matching pfettrans.*.csv"
        return 1
    fi
...
clean_files() {
    ...
    rm -f pfettrans.*.csv pfettrans.out pfettrans.*.png pfettrans_currents.png pfettrans_comparison.png
    echo_status "Done"
}
...
case "$MODE" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  run   - Run simulation and save as pfettrans.HOSTNAME.csv (default)"
        echo "  plot  - Plot comparison of all pfettrans.*.csv files"
