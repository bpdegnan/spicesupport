#!/usr/bin/env python3
"""
Plot PFET transient gatesweep with all terminal currents.
Supports comparison across multiple hosts.

Usage:
    python3 plot_transient_currents.py pfettrans.host1.csv
    python3 plot_transient_currents.py pfettrans.*.csv -o comparison.png
"""
...
def extract_hostname(filepath):
    """Extract hostname from filename like pfettrans.gminXX.hostname.csv or pfettrans.hostname.csv"""
    basename = os.path.basename(filepath)
    # Try new format: pfettrans.gminXX.hostname.csv
    match = re.match(r'pfettrans\.gmin[^.]+\.(.+)\.csv', basename)
    if match:
        return match.group(1)
    # Try old format: pfettrans.hostname.csv
    match = re.match(r'pfettrans\.(.+)\.csv', basename)
    if match:
        return match.group(1)
    return os.path.splitext(basename)[0]

def extract_gmin_from_filename(filepath):
    """Extract gmin value from filename like pfettrans.gminXX.hostname.csv"""
    basename = os.path.basename(filepath)
    match = re.match(r'pfettrans\.(gmin[^.]+)\.', basename)
    if match:
        return match.group(1)
    return None
...
def plot_terminal_currents(csv_files, output_file='pfettrans_currents.png'):
    ...
    ax_curr.set_title('PFET Terminal Currents')
...
def main():
    parser = argparse.ArgumentParser(description='Plot PFET terminal currents')
    ...
    if args.output is None:
        ...
        if gmin_str:
            args.output = f'pfettrans.{gmin_str}.png'
        else:
            args.output = 'pfettrans_currents.png'
