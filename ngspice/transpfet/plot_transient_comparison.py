#!/usr/bin/env python3
"""
Plot comparison of pfettrans results from multiple hosts.

Usage:
    python3 plot_transient_comparison.py pfettrans.host1.csv pfettrans.host2.csv ...
    python3 plot_transient_comparison.py pfettrans.*.csv -o comparison.png
"""
...
def extract_hostname(filepath):
    """Extract hostname from filename like pfettrans.hostname.csv"""
...
def plot_comparison(csv_files, output_file='pfettrans_comparison.png'):
    ...
    ax1.set_ylabel('Vg (V)')
    ax1.set_title('Gate Voltage vs Time (PFET: VDD → 0)')
...
def main():
    parser = argparse.ArgumentParser(description='Plot pfettrans host comparison')
    parser.add_argument('csv_files', nargs='+', help='CSV files to compare')
    parser.add_argument('-o', '--output', default='pfettrans_comparison.png', 
