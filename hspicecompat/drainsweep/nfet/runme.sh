#!/usr/bin/env zsh
# Run HSPICE and ngspice NFET drain sweep and compare results
...
run_hspice() {
    echo_status "Running HSPICE NFET drain sweep..."
    check_hspice || return 1
    
    "$HSPICE_BIN" nfet_drainsweephspice.cir > nfet_drainsweephspice.out 2>&1
    
    if grep -q "job concluded" nfet_drainsweephspice.out; then
        echo_status "HSPICE completed successfully"
        "$PYTHON_BIN" hspice_dc_to_csv.py nfet_drainsweephspice.out nfet_drainsweep_hspice.csv
        echo_status "Created nfet_drainsweep_hspice.csv"
    else
        echo_error "HSPICE failed - check nfet_drainsweephspice.out"
        tail -20 nfet_drainsweephspice.out
        return 1
    fi
}

run_ngspice() {
    echo_status "Running ngspice NFET drain sweep..."
    check_ngspice || return 1
    check_spice_lib || return 1
    
    "$NGSPICE_BIN" -b nfet_drainsweepngspice.cir > nfet_drainsweepngspice.out 2>&1
    
    if grep -q "Done" nfet_drainsweepngspice.out && [[ -f "nfet_drainsweep_ngspice.csv" ]]; then
        echo_status "ngspice completed successfully"
        echo_status "Created nfet_drainsweep_ngspice.csv"
    else
        echo_error "ngspice failed - check nfet_drainsweepngspice.out"
        cat nfet_drainsweepngspice.out
        return 1
    fi
}
...
plot_results() {
    ...
    PLOT_ARGS=()
    [[ -f "nfet_drainsweep_hspice.csv" ]] && PLOT_ARGS+=(--hspice nfet_drainsweep_hspice.csv) && echo_status "  Including HSPICE data"
    [[ -f "nfet_drainsweep_ngspice.csv" ]] && PLOT_ARGS+=(--ngspice nfet_drainsweep_ngspice.csv) && echo_status "  Including ngspice data"
    
    if [[ ${#PLOT_ARGS[@]} -gt 0 ]]; then
        "$PYTHON_BIN" plot_nfet_drainsweep_comparison.py "${PLOT_ARGS[@]}" -o nfet_drainsweep_comparison.png
        echo_status "Created nfet_drainsweep_comparison.png"
    else
