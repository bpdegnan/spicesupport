#!/usr/bin/env zsh
# Run HSPICE and ngspice PFET gate sweep and compare results
...
run_hspice() {
    echo_status "Running HSPICE PFET gate sweep..."
    check_hspice || return 1
    
    "$HSPICE_BIN" pfethspice.cir > pfethspice.out 2>&1
    
    if grep -q "job concluded" pfethspice.out; then
        echo_status "HSPICE completed successfully"
        "$PYTHON_BIN" hspice_dc_to_csv.py pfethspice.out pfet_hspice.csv
        echo_status "Created pfet_hspice.csv"
    else
        echo_error "HSPICE failed - check pfethspice.out"
        tail -20 pfethspice.out
        return 1
    fi
}

run_ngspice() {
    echo_status "Running ngspice PFET gate sweep..."
    check_ngspice || return 1
    check_spice_lib || return 1
    
    "$NGSPICE_BIN" -b pfetngspice.cir > pfetngspice.out 2>&1
    
    if grep -q "Done" pfetngspice.out && [[ -f "pfet_ngspice.csv" ]]; then
        echo_status "ngspice completed successfully"
        echo_status "Created pfet_ngspice.csv"
    else
        echo_error "ngspice failed - check pfetngspice.out"
        cat pfetngspice.out
        return 1
    fi
}
...
plot_results() {
    ...
    PLOT_ARGS=()
    [[ -f "pfet_hspice.csv" ]] && PLOT_ARGS+=(--hspice pfet_hspice.csv) && echo_status "  Including HSPICE data"
    [[ -f "pfet_ngspice.csv" ]] && PLOT_ARGS+=(--ngspice pfet_ngspice.csv) && echo_status "  Including ngspice data"
    
    if [[ ${#PLOT_ARGS[@]} -gt 0 ]]; then
        "$PYTHON_BIN" plot_pfet_comparison.py "${PLOT_ARGS[@]}" -o pfet_comparison.png
        echo_status "Created pfet_comparison.png"
    else
